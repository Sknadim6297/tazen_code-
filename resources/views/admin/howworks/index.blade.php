            ...
            <div class="col-xxl-12 col-xl-12">
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        ...
                        <div class="d-flex">
                            <button class="btn btn-sm btn-primary btn-wave waves-light" data-bs-toggle="modal" data-bs-target="#create-task"><i class="ri-add-line fw-medium align-middle me-1"></i> Add How It Works</button>
                            <!-- Start::add task modal -->
                            <div class="modal fade" id="create-task" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <form action="{{ route('admin.howworks.store') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h6 class="modal-title">Add How It Works Section</h6>
                                                ...
                                                <div class="row gy-3">
                                                    ...
                                                    @for ($i = 1; $i <= 3; $i++)
                                                        ...
                                                        <div class="col-md-4">
                                                            ...
                                                            <input type="file" class="form-control" name="image{{ $i }}" id="image{{ $i }}" accept="image/*" required>
                                                        </div>
                                    
                                                        {{-- Heading --}}
                                                        <div class="col-md-4">
                                                            <label for="heading{{ $i }}" class="form-label">Heading {{ $i }}</label>
                                                            <input type="text" class="form-control" name="heading{{ $i }}" id="heading{{ $i }}" placeholder="Enter Heading {{ $i }}" required>
                                                        </div>
                                    
                                                        {{-- Description --}}
                                                        <div class="col-md-4">
                                                            <label for="description{{ $i }}" class="form-label">Description {{ $i }}</label>
                                                            <textarea class="form-control" name="description{{ $i }}" id="description{{ $i }}" rows="2" placeholder="Enter Description {{ $i }}" required></textarea>
                                                        </div>
                                                    @endfor
                                                </div>
                                            </div>
                            ...
                            <table class="table text-nowrap">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.howworks.edit', $item->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                            <form action="{{ route('admin.howworks.destroy', $item->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
