...
<style>
    ...
    .form-control:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(79, 70, 229, 0.12); outline: none; }
    ...
    .btn-primary { background: linear-gradient(135deg, var(--primary), var(--primary-dark)); color: #ffffff; box-shadow: 0 18px 40px rgba(79, 70, 229, 0.22); }
    .btn-neutral { background: rgba(148, 163, 184, 0.18); color: #0f172a; border: 1px solid rgba(148, 163, 184, 0.35); }
    .btn-outline { background: transparent; color: var(--muted); border: 1px solid rgba(148, 163, 184, 0.35); }
    ...
    @media (max-width: 768px) {
        .requested-edit-page { padding: 2.2rem 1rem 3.2rem; }
        .edit-hero { padding: 1.75rem 1.6rem; }
...
@section('content')
<div class="requested-edit-page">
    <div class="requested-edit-shell">
        <section class="edit-hero">
            <div class="hero-meta">
                <span class="hero-eyebrow"><i class="fas fa-clipboard-list"></i>Information</span>
                <h1>Edit Other Information</h1>
        ...
        <section class="form-card">
            ...
            <form id="serviceForm">
                ...
                <div class="form-group">
                    ...
                    <textarea name="sub_heading" id="sub_heading" class="form-control" placeholder="Enter services details..." required>{{ $info->sub_heading }}</textarea>
                </div>
                    ...
                    @foreach(json_decode($info->requested_service) as $key => $service)
                        <div class="dynamic-group requested-service">
                            <div class="form-group">
                                ...
                                <input type="text" name="requested_service[]" class="form-control" value="{{ $service }}" placeholder="Add service you can offer" required>
                            </div>
                            <div class="form-group">
                                <label>Price *</label>
                                <input type="number" name="price[]" class="form-control" value="{{ json_decode($info->price)[$key] }}" step="0.01" placeholder="Enter price" required>
                            </div>
                            @if($key > 0)
                ...
                <div id="educationContainer" style="display:flex; flex-direction:column; gap:1rem;">
                    @php $education = json_decode($info->education, true); @endphp
